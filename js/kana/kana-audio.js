...
export async function start(container, deck, allItems) {
  showSessionConfig(container, {
    ...
    subtitle: `Oye el ${deck === 'hiragana' ? 'hiragana' : 'katakana'} y elige el carácter correcto.`,
    groups: GROUPS,
    onStart: (size, groups) => {
      ...
      runAudio(container, deck, () => selectSession(deck, filtered, size), allItems);
    },
  });
}

function runAudio(container, deck, getItems, allItems) {
  startExercise(container, {
    ...
    renderPrompt(item, el) {
      el.innerHTML = `
        <div class="audio-prompt">
          <div class="prompt-hint">¿Qué carácter es?</div>
    ...
    checkAnswer: (item, answer) => item.kana === answer,
    getCorrectDisplay: item => `${item.kana} (${item.romaji})`,
